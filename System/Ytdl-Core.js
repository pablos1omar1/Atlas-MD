...
class YT {
  ...
  static downloadMusic = async (query) => {
    try {
      ...
      //   { lang: "id" }
      // );
      const videoInfo = await youtubedl("https://www.youtube.com/watch?v=" + search.id, {
        dumpSingleJson: true,  // Fetch metadata as JSON
        noWarnings: true,
        noCheckCertificate: true,
        preferFreeFormats: true,
        skipDownload: true  // Prevents downloading the video
    });
      let stream = await youtubedl(url, {
        output: songPath,
        extractAudio: true,
        audioFormat: 'mp3',
      ...
      await this.WriteTags(file, {
        ...
        Image: search.image,
        Album: search.album,
        Year: videoInfo.upload_date.slice(0, 4),
      });
      return {
        meta: search,
        path: songPath,
    ...
    try {
      ...
      if (autoWriteTags) {
        await this.WriteTags(file, {
          ...
          Album: videoDetails.uploader,
          Year: videoDetails.upload_date.slice(0, 4),
